...
void update_telemetry_and_blackbox(WritePWM& motor, float gz_rate, float dt_ekf, float dt_pid, uint64_t end_time_us, bool& blackbox_updated) {
    ...
    if ((end_time_us - last_update_print_us) > 1000000/60) {
        ...
        DEBUG_PRINT("Roll: %.2f, Pitch: %.2f, Yaw: %.2f, RC_Roll: %f, RC_Pitch: %f, RC_Yaw: %f, PID_Roll: %f, PID_Pitch: %f, PID_Yaw: %f, dt_pid: %f, dt_ekf: %f\n",
                    roll, pitch, gz_rate, receiver_pwm[0], receiver_pwm[1], receiver_pwm[3], roll_control_output, pitch_control_output, yaw_control_output,
                    dt_pid, dt_ekf);
        last_update_print_us = end_time_us;
    }
}

void handle_disarmed_state(WritePWM& motor, QuaternionEKF& filter, bool& blackbox_updated, bool& blackbox_dumped, bool& first_throttle_on) {
    motor.reset();
