...
void BMI160::writeRegister(uint8_t reg, uint8_t data) {
    uint8_t buf[2];
...
void BMI160::readRegisters(uint8_t reg, uint8_t* buffer, uint16_t length) {
...
void BMI160::readData(float* ax, float* ay, float* az, float* gx, float* gy, float* gz) {
    uint8_t buffer[12];
