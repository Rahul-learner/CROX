...
#define ANGLE_TUNE_HEADER_1 0xCC
#define ANGLE_TUNE_HEADER_2 0x77

struct AngleTuningPacket {
    uint8_t header1;
    uint8_t header2;
    int16_t angle_strength;  // scaled by 1000
    int16_t angle_max_deg;   // scaled by 10
    int16_t angle_max_rate;  // scaled by 10
    uint8_t reserved[23];    // Pad to 32 bytes (2+6=8 used, 24 padding? Wait: 2 headers + 3*2 = 8 bytes. 32-8-1=23 bytes.)
    uint8_t checksum;
};
...
public:
    ...
    // Reads whatever is in the RX payload and routes it to the correct struct
    int readCommand(PIDTuningPacket* pid_data, AngleTuningPacket* angle_data) {
        uint8_t buffer[32] = {0};
        ...
        if (buffer[0] == PID_HEADER_1 && buffer[1] == PID_HEADER_2) {
            uint8_t calc_cs = 0;
            for (size_t i = 0; i < sizeof(PIDTuningPacket) - 1; i++) calc_cs ^= buffer[i];
            
            if (calc_cs == buffer[31]) {
                memcpy(pid_data, buffer, sizeof(PIDTuningPacket));
                return 1; // PID
            } else {
                printf("[NRF24 ERROR] PID Checksum Mismatch!\n");
            }
        } else if (buffer[0] == ANGLE_TUNE_HEADER_1 && buffer[1] == ANGLE_TUNE_HEADER_2) {
            uint8_t calc_cs = 0;
            for (size_t i = 0; i < sizeof(AngleTuningPacket) - 1; i++) calc_cs ^= buffer[i];

            if (calc_cs == buffer[31]) {
                memcpy(angle_data, buffer, sizeof(AngleTuningPacket));
                return 2; // Angle
            } else {
                printf("[NRF24 ERROR] Angle Checksum Mismatch!\n");
            }
        } else {
             // Not a valid command packet
        }

        writeCmd(FLUSH_RX);
        return 0; // Unknown or corrupt
    }
    ...
    bool sendAngleTune(AngleTuningPacket* data) {
        if (!isReady()) {
            printf("[NRF24 ERROR] Radio busy, skipping Angle packet\n");
            return false;
        }

        data->header1 = ANGLE_TUNE_HEADER_1;
        data->header2 = ANGLE_TUNE_HEADER_2;

        uint8_t calc_cs = 0;
        uint8_t* ptr = (uint8_t*)data;
        for (size_t i = 0; i < sizeof(AngleTuningPacket) - 1; i++) calc_cs ^= ptr[i];
        data->checksum = calc_cs;
        ...
        if (status & 0x10) {
            printf("[NRF24 ERROR] Angle TX MAX_RT (Max Retries) hit! Drone offline.\n");
            writeCmd(FLUSH_TX);
        }
