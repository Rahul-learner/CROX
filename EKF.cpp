...
void QuaternionEKF::predict(float gx, float gy, float gz, float dt, float q_quat, float q_bias) {
...
void QuaternionEKF::update(float ax, float ay, float az, float r_accel) {
...
void QuaternionEKF::getEulerAngles(float &roll_deg, float &pitch_deg, float &yaw_deg) {
    float q0 = x[0], q1 = x[1], q2 = x[2], q3 = x[3];

    roll_deg  = std::atan2(2.0f * (q0*q1 + q2*q3), 1.0f - 2.0f * (q1*q1 + q2*q2)) / DEG_TO_RAD;
    float sinp = 2.0f * (q0 * q2 - q3 * q1);
    if (std::abs(sinp) >= 1)
        pitch_deg = std::copysign(M_PI/2.0f, sinp)*(180.0f/M_PI); // Use 90 degrees if out of range
    else
        pitch_deg = std::asin(sinp) * (180.0f / M_PI);
...
void MahonyFilter::update(float ax, float ay, float az, float gx, float gy, float gz, float dt) {
    // Normalise accelerometer measurement
    float recipNorm = 1.0f / std::sqrt(ax * ax + ay * ay + az * az);
    ax *= recipNorm;
    ay *= recipNorm;
    az *= recipNorm;
...
float MahonyFilter::getRoll() {
    return std::atan2(2.0f * (q0 * q1 + q2 * q3), 1.0f - 2.0f * (q1 * q1 + q2 * q2)) * 180.0f / M_PI;
}

float MahonyFilter::getPitch() {
    return std::asin(2.0f * (q0 * q2 - q3 * q1)) * 180.0f / M_PI;
}
